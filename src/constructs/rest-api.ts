...
export interface RestApiProps<OPS> extends BaseApiProps {
  ...
  /**
   * OAuth2 scopes required by the Cognito authorizer.
   *
   * When a Cognito authorizer is configured with scopes, API Gateway validates the
   * **access token** and checks that it contains at least one of the specified scopes.
   * When no scopes are configured (empty array), API Gateway validates the **ID token**
   * instead, rejecting access tokens with a 401.
   *
   * Use this when your application authorizes via access tokens — for example, when
   * using `preTokenGenerationv2` to inject custom claims into the access token.
   *
   * Provide a string array for a global default applied to all secured operations,
   * or an object keyed by operationId for per-operation overrides.
   *
   * @example
   * // Global scopes — all secured operations require 'openid'
   ...
   * }
   *
   * @default [] (empty — ID-token-only validation)
   */
  authorizationScopes?: string[] | { [operationId in keyof OPS]?: string[] };
}
...
export class RestApi<PATHS, OPS> extends BaseApi {
  ...
  protected patchSecuritySchemes(spec: OpenAPI3) {
    ...
    if (this.props.authentication.hasOwnProperty('userpool')) {
      ...
      const globalScopes = Array.isArray(this.props.authorizationScopes)
  ...
  protected patchSecurity(spec: OpenAPI3) {
    if ('security' in spec) {
      // Resolve per-operation scope overrides when authorizationScopes is an object map.
      const perOperationScopes: Record<string, string[]> | undefined =
        this.props.authorizationScopes && !Array.isArray(this.props.authorizationScopes)
          ? this.props.authorizationScopes as Record<string, string[]>
          : undefined;
